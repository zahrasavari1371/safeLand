...
function initializeDatepicker() {
  ...
  const datepicker = new Datepicker(datepickerInput, {
    ...
    language: "en",
    weekStart: 0,
    maxDate: new Date(2024, 11, 31), // Example max date
    minDate: new Date(2020, 0, 1), // Example min date
    datesDisabled: [], // Example disabled dates
    daysOfWeekDisabled: [], // Example disabled days of week
    daysOfWeekHighlighted: [], // Example highlighted days of week
    todayBtn: true,
    todayHighlight: true,
    clearBtn: true,
    title: "Select Date",
...
// Initialize datepicker when the DOM is fully loaded
...
const sideNavModeList = [
  "side-nav-transparent",
  "side-nav-light",
  "side-nav-dark",
  "side-nav-themed",
];
const stackedSideNavMiniModeList = [
  "stacked-side-nav-mini-light",
  "stacked-side-nav-mini-dark",
  "stacked-side-nav-mini-themed",
];
const secondaryNavModeList = [
  "secondary-header-transparent",
...
function chainRemoveClass(selector, classList) {
  $(selector).removeClass(classList.join(" "));
}
...
// Main theme configurator function
function themeConfigurator() {
  let direction = "rtl"; // Default direction
  ...
  // Function to set logo based on conditions
  function setLogo() {
    const logo = $(".logo");
    const logoImg = $(".logo img");
    const isDarkMode = $("html").hasClass("dark");
    const isCollapse = sideNav.hasClass("side-nav-collapsed");
    const isThemedSideNav =
      sideNav.hasClass("side-nav-themed") ||
      stackedSideNavMini.hasClass("stacked-side-nav-mini-themed");
    const logoType =
      isCollapse || hasElement(".stacked-side-nav") ? "streamline" : "full";
    const logoMode = isDarkMode || isThemedSideNav ? "dark" : "light";

    if (isCollapse || hasElement(".stacked-side-nav")) {
      logo.removeClass("px-6").addClass("px-4");
    } else {
      logo.removeClass("px-4").addClass("px-6");
    }

    logoImg.attr("src", `img/logo/logo-${logoMode}-${logoType}.png`);
  }

  // Function to reset side navigation modes
  function resetSideNavMode() {
    chainRemoveClass(".side-nav", sideNavModeList);
    chainRemoveClass(".side-nav .side-nav-content .menu", menuModeList);
  }

  // Function to reset secondary navigation modes
  function resetSecondaryNavMode() {
    chainRemoveClass(".secondary-header", secondaryNavModeList);
  }

  // Function to reset stacked side navigation mini modes
  function resetStackedSideNavMini() {
  ...
  // Function to set default navigation mode based on URL path
  function setDefaultNavMode() {
    ...
    function setStackedSideNavMode(mode) {
    ...
    if (
      ...
      currentPath.includes("stackedSide")
    ) {
      resetSideNavMode();
  ...
  $(document).ready(function () {
    $('input[name="dark-mode-toggle"]').on("change", function (e) {
      const mode = e.target.checked ? "dark" : "light";
      const oppositeMode = mode === "light" ? "dark" : "light";

      const darkModeDateElement = document.getElementById("dark-mode-date");
      if (darkModeDateElement) {
        if (e.target.checked) {
          darkModeDateElement.className = "locale-fa-dark-mode";
        } else {
          $(darkModeDateElement).removeClass("locale-fa-dark-mode");
        }
      }

      $("html").removeClass(oppositeMode).addClass(mode);
      ...
      if (hasElement(stackedSideNav)) {
        ...
        if (
          !$(".stacked-side-nav-mini").hasClass("stacked-side-nav-mini-themed")
        ) {
          resetStackedSideNavMini();
          $(".stacked-side-nav-mini .menu").addClass(`menu-${mode}`);
  ...
  $("#dir-ltr-button").on("click", function () {
    if (direction !== "ltr") {
      ...
      $("#dir-ltr-button")
        ...
        .addClass("bg-gray-100 dark:bg-gray-500");
  ...
  // تابع برای بروزرسانی رنگ‌ها
  function updateThemeColors(color) {
    const colorLevels = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];
    let primary =
      color === "primary" ? themeColors["indigo"] : themeColors[color];

    if (!primary) {
      console.warn(`Color "${color}" is not defined in themeColors.`);
      return;
    }
  ...
  // فراخوانی تابع هنگام تغییر انتخاب در <select>
  ...
  // اضافه کردن رویدادها به دکمه‌های تغییر لایوت
  document.querySelectorAll("[id^=layout-]").forEach((button) => {
    button.addEventListener("click", () => {
      handleLayoutClick(button.id.replace("layout-", ""));
    });
  });

  // تغییر حالت ناوبری
  document
    .querySelectorAll('input[name="nav-mode-radio-group"]')
    .forEach((input) => {
      input.addEventListener("change", function () {
        ...
        if (selectedValue === "themed") {
          if (sideNav) {
            resetSideNavMode();
            sideNav.classList.add("side-nav-themed");
            sideNavMenu.classList.add("menu-themed");
          }

          if (secondaryHeader) {
            resetSecondaryNavMode();
            secondaryHeader.classList.add("secondary-header-themed");
          }

          if (stackedSideNav) {
            resetStackedSideNavMini();
            document
              .querySelectorAll(".stacked-side-nav-mini .menu")
              .forEach((menu) => {
                menu.classList.add("menu-themed");
              });
            document
              .querySelectorAll(".stacked-side-nav-mini")
              .forEach((nav) => {
                nav.classList.add("stacked-side-nav-mini-themed");
              });
          }
  ...
  // مقداردهی اولیه لوگو و حالت پیش‌فرض ناوبری
  setLogo();
  setDefaultNavMode();
}

// Call the themeConfigurator function when document is ready
$(document).ready(function () {
  themeConfigurator();
});
///////////////////////////////////////////////////////////////////////
...
(function () {
  ...
  function dim(input) {
    return picocolors.dim(input);
  }

  const logUtils = {
    info(key, messages) {
      log(
        picocolors.bold(picocolors.cyan("info")),
        Array.isArray(key) ? [key] : [messages, key]
      );
    },
    warn(key, messages) {
      log(
        picocolors.bold(picocolors.yellow("warn")),
        Array.isArray(key) ? [key] : [messages, key]
      );
    },
    risk(key, messages) {
      log(
        picocolors.bold(picocolors.magenta("risk")),
        Array.isArray(key) ? [key] : [messages, key]
      );
    },
  };

  // Make logUtils globally available
