...
function makeMoment(input, format, lang, strict, utc) {
  ...
  if (
    config._isValid === false ||
    (input && input._isAMomentObject && !input._isValid)
  ) {
    m._isValid = false;
  }
...
jMoment.fn.diff = function (input, unitOfTime, asFloat) {
  ...
  function monthDiff(a, b) {
    ...
    var wholeMonthDiff =
        (b.jYear() - a.jYear()) * 12 + (b.jMonth() - a.jMonth()),
...
jMoment.fn.isSame = function (other, units) {
  ...
  if (units === "jyear" || units === "jmonth") {
    return moment.fn.isSame.call(
      this.clone().startOf(units),
      other.clone().startOf(units)
    );
  }
...
jMoment.fn.isBefore = function (other, units) {
  units = normalizeUnits(units, this);
  if (units === "jyear" || units === "jmonth") {
    return moment.fn.isBefore.call(
      this.clone().startOf(units),
      other.clone().startOf(units)
    );
  }
...
jMoment.jConvert = { toJalali: toJalali, toGregorian: toGregorian };
